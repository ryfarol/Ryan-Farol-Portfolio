...
def outputcontent(paragraphs, primary, secondary):
    ...
    check = len(paragraphs) # initialize how many paragraphs are on the wiki page
    for search in paragraphs:  # iterate through all the paragraphs until both words are found
        ...
        elif prim_lower and sec_lower not in search.text.lower(): # count how paragraphs that don't have the keyword
            counter += 1
    if counter == check: # if all paragraphs dont have the keywords, search cannot be found
        result = "Search Not Found. Please Try Again!"
        text_box = tk.Text(root, height=10, width=50, padx=15, pady=15)
        text_box.insert(1.0, result)
        text_box.grid(column=1, row=10)
        download_button = tk.Button(root, text="Download CSV", command=lambda: outputCSV(primary, secondary, result), font="Arial",
